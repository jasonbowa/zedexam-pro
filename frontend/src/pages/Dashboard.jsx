...
const normalizeList = (value) => {
  ...
  if (Array.isArray(value?.mockExams)) return value.mockExams;
  if (Array.isArray(value?.data)) return value.data;
  if (Array.isArray(value?.data?.subjects)) return value.data.subjects;
  if (Array.isArray(value?.data?.results)) return value.data.results;
  if (Array.isArray(value?.data?.mockExams)) return value.data.mockExams;
  return [];
};
...
export default function Dashboard() {
  ...
  useEffect(() => {
    const loadDashboard = async () => {
      ...
      const [subjectsData, resultsData, mockData, subscriptionData] =
      ...
      const loadedSubjects =
        subjectsData.status === "fulfilled"
          ? normalizeList(subjectsData.value)
          : [];
      const loadedResults =
        resultsData.status === "fulfilled"
          ? normalizeList(resultsData.value)
          : [];
      const loadedMockExams =
        mockData.status === "fulfilled"
          ? normalizeList(mockData.value)
          : [];
      ...
      if (subjectsData.status === "rejected") {
        setError(subjectsData.reason?.message || "Failed to load subjects.");
      }
  ...
  const averageScore = useMemo(() => {
    ...
    const total = results.reduce((sum, item) => {
      ...
      return (
        sum + (rawTotal > 0 ? Math.round((rawScore / rawTotal) * 100) : rawScore)
      );
    }, 0);
  ...
  const certificatesEarned = useMemo(
    () =>
      results.filter((item) => {
        ...
        const rawTotal = Number(item.total || item.totalQuestions || 0);
        const percent =
          rawTotal > 0 ? Math.round((rawScore / rawTotal) * 100) : rawScore;
        return percent >= 50;
      }).length,
    [results]
  );
    ...
    >
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
        <StatCard
          ...
          hint="Available learning areas"
          accent="slate"
        />
        <StatCard
          label="Attempts"
          value={loading ? "..." : results.length}
          hint="Completed quiz submissions"
          accent="blue"
        />
        <StatCard
          label="Average Score"
          value={loading ? "..." : `${averageScore}%`}
          hint="Overall student performance"
          accent="emerald"
        />
        <StatCard
          label="Certificates"
          value={loading ? "..." : certificatesEarned}
          hint="Passed with 50% or above"
          accent="amber"
        />
        <StatCard
          label="Plan"
          value={
            loading
              ? "..."
              : subscription?.package?.name || subscription?.name || "Free"
          }
          hint={subscription?.status || "No active paid plan"}
          accent="white"
        />
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
        <SectionCard
          ...
          subtitle="Choose a subject and keep building mastery topic by topic."
        >
          {loading ? (
            ...
            <EmptyState
              ...
              text="Once subjects are available, they will appear here for students to begin learning."
            />
          ) : (
              ...
              {subjects.slice(0, 6).map((subject) => (
                ...
                >
                  ...
                  <p className="mt-2 text-sm leading-6 text-slate-500">
                    {subject.description ||
                      "Practice questions, revision topics, and structured exam preparation."}
                  </p>
        ...
        <SectionCard
          ...
          subtitle="Quick next steps to help the student move forward."
        >
          <div className="space-y-4">
            <div className="rounded-[24px] bg-slate-50 p-5 ring-1 ring-slate-200">
              ...
              <p className="mt-1 text-sm text-slate-500">
                {loading
                  ? "..."
                  : `${mockExams.length} timed mock exam(s) available for practice.`}
              </p>
            </div>
            ...
            <div className="rounded-[24px] bg-slate-50 p-5 ring-1 ring-slate-200">
              ...
              <p className="mt-1 text-sm text-slate-500">
                {results.length
                  ? "Review recent results, then return to weak topics."
                  : "Start with a subject and complete your first quiz attempt."}
              </p>
            </div>
            ...
            <div className="rounded-[24px] bg-slate-50 p-5 ring-1 ring-slate-200">
              ...
              <p className="mt-1 text-sm text-slate-500">
                {subscription?.package?.name || subscription?.name
                  ? `${
                      subscription?.package?.name || subscription?.name
                    } • ${subscription?.status || "active"}`
                  : "You are on the free plan until a package is assigned or purchased."}
              </p>
            </div>
          </div>
        </SectionCard>
      ...
      <SectionCard
        ...
        subtitle="See performance, status, and certificate eligibility at a glance."
      >
        {loading ? (
          ...
          <EmptyState
            ...
            text="When the student completes quizzes, results will appear here with the option to open details and certificates."
            action={
                  ...
                  return (
                    <tr key={result.id} className="border-b border-slate-100">
                      ...
                      <td className="px-4 py-4">
                        <span
                          className={`badge ${
                            passed ? "badge-success" : "badge-warning"
                          }`}
                        >
