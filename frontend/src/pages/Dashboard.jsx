...
export default function Dashboard() {
  ...
  const hasActiveSubscription = String(subscriptionStatus).toUpperCase() === 'ACTIVE';
  const pendingActivation = !hasActiveSubscription;
    ...
    <PageShell
      ...
      action={
        <>
          ...
          <Link to="/notes" className="btn border border-white/20 bg-white/10 text-white hover:bg-white/20">Online Notes</Link>
          <Link to="/mock-exams" className="btn border border-white/20 bg-white/10 text-white hover:bg-white/20">Mock Exams</Link>
          <Link to="/packages" className="btn border border-white/20 bg-white/10 text-white hover:bg-white/20">Packages</Link>
        </>
      }
    >
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <StatCard label="Plan" value={loading ? '...' : subscription?.package?.name || 'Free'} hint={subscriptionStatus} accent="white" />
      </div>

      {!loading && pendingActivation ? (
        <>
          <Notice tone="warning">
            Your account is created, but package access is still pending. Use the payment instructions below, then send proof on WhatsApp so admin can activate your package.
          </Notice>
          <PaymentInstructions status={subscriptionStatus} packageName={subscription?.package?.name || 'Student package'} />
          <SectionCard title="What unlocks after activation" subtitle="This keeps the learner experience clear while payment is being confirmed.">
            <div className="grid gap-3 md:grid-cols-3">
              {['Package-controlled subjects and quizzes', 'Mock exams and results tracking', 'Online notes, eligible PDFs, and certificates where included'].map((item) => (
                <div key={item} className="rounded-2xl bg-white p-4 text-sm font-semibold text-slate-700 ring-1 ring-slate-200">{item}</div>
              ))}
            </div>
          </SectionCard>
        </>
        ...
        <SectionCard title="Platform Snapshot" subtitle="Quick next steps to help the student move forward.">
          <div className="space-y-4">
            ...
            <div className="rounded-[24px] bg-slate-50 p-5 ring-1 ring-slate-200">
              ...
              <p className="mt-1 text-sm text-slate-500">{subscription?.package?.name ? `${subscription.package.name} / ${subscription.status}` : 'No active paid package yet. Use the Packages page for payment instructions.'}</p>
            </div>
          </div>
        </SectionCard>
      </div>
      ...
      <SectionCard title="Account Security" subtitle="Change your password when you still know your current one. If you forgot it, admin can reset it from Students Management.">
